...
async def root():
    return {
        ...
        "version": "1.0.0",
        "deployment_method": "GitHub Actions"  # Indicate we use Actions
    }
...
async def health_check():
    ...
    health_status = {
        "status": "healthy",
        "timestamp": datetime.now().isoformat(),
        "deployment_method": "github_actions",  # Matches diagram
        "components": {
            "api": "ok",
            "github": "unknown",
            "openai": "unknown"
...
async def process_deployment(request_data: dict, is_update: bool = False):
    ...
    try:
        ...
        if is_update:
            ...
            # Pages should already be configured, but trigger workflow
            ...
            # Enable GitHub Pages using GitHub Actions (matches diagram)
            pages_url = github_client.enable_pages_with_actions(repo)
...
async def update_app(request: DeploymentRequest, background_tasks: BackgroundTasks):
    ...
    return DeploymentResponse(
        status="processing",
        message=f"Update started successfully for round {request.round}. The application is being updated via GitHub Actions.",
        task_id=request.task,
        timestamp=datetime.now().isoformat()
    )
